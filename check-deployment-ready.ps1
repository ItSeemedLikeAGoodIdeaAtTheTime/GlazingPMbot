...
if ($account) {
    Write-Host "  $([char]0x2713) Authenticated as: $account" -ForegroundColor Green
} else {
    Write-Host "  $([char]0x2717) Not authenticated" -ForegroundColor Red
...
if ($project) {
    Write-Host "  $([char]0x2713) Project: $project" -ForegroundColor Green
} else {
    Write-Host "  $([char]0x2717) No project set" -ForegroundColor Red
...
if ($env:anthropicAPIkey) {
    ...
    Write-Host "  $([char]0x2713) API key found: $keyPreview" -ForegroundColor Green
} elseif ($env:ANTHROPIC_API_KEY) {
    $keyPreview = $env:ANTHROPIC_API_KEY.Substring(0, 10) + "..." + $env:ANTHROPIC_API_KEY.Substring($env:ANTHROPIC_API_KEY.Length - 4)
    Write-Host "  $([char]0x2713) API key found: $keyPreview" -ForegroundColor Green
} else {
    Write-Host "  $([char]0x2717) Anthropic API key not found in environment" -ForegroundColor Red
    Write-Host "     You will need to provide it during deployment" -ForegroundColor Yellow
}

# Check 5: Docker files exist
Write-Host "`n[5/5] Checking deployment files..." -ForegroundColor Yellow
if (Test-Path "api/Dockerfile") {
    Write-Host "  $([char]0x2713) Dockerfile exists" -ForegroundColor Green
} else {
    Write-Host "  $([char]0x2717) Dockerfile missing" -ForegroundColor Red
    $ready = $false
}

if (Test-Path "api/main.py") {
    Write-Host "  $([char]0x2713) API code exists" -ForegroundColor Green
} else {
    Write-Host "  $([char]0x2717) API code missing" -ForegroundColor Red
    $ready = $false
}
...
if ($ready) {
    ...
    Write-Host "  $([char]0x2717) NOT READY - Fix issues above" -ForegroundColor Red
    Write-Host "========================================`n" -ForegroundColor Cyan
}
